...
async def test_full_user_management_journey(client: AsyncClient, db_session: AsyncSession) -> None:
    ...
    reg_user = await client.post(
        "/api/v1/auth/register", json={"email": user_email, "password": password}
    )
    ...
    users_list = await client.get("/api/v1/users", headers=admin_headers)
    assert users_list.status_code == 200
    assert users_list.json()["total"] >= 2

    # Adım 6: Sıradan kullanıcıyı ID ile getir
    get_user = await client.get(f"/api/v1/users/{user_id}", headers=admin_headers)
    ...
    forbidden = await client.get("/api/v1/users", headers=non_admin_headers)
