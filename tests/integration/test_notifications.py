"""Notifications endpoint testleri — /api/v1/notifications"""
...
async def test_list_notifications_pagination(client: AsyncClient, db_session: AsyncSession):
    ...
    res = await client.get("/api/v1/notifications?page=1&size=3", headers=headers)
...
async def test_mark_notification_read(client: AsyncClient, db_session: AsyncSession):
    ...
    res = await client.patch(f"/api/v1/notifications/{notif_id}", headers=headers)
...
async def test_mark_notification_read_decreases_unread_count(
    ...
    count_before = (await client.get("/api/v1/notifications/unread-count", headers=headers)).json()[
        "count"
    ]
    await client.patch(f"/api/v1/notifications/{notif_id}", headers=headers)
    count_after = (await client.get("/api/v1/notifications/unread-count", headers=headers)).json()[
        "count"
    ]
...
async def test_mark_other_users_notification_fails(client: AsyncClient, db_session: AsyncSession):
    ...
    user_a_id = (await client.get("/api/v1/users/me", headers=headers_a)).json()["id"]
    ...
    res = await client.patch(f"/api/v1/notifications/{notif_id}", headers=headers_b)
...
async def test_mark_nonexistent_notification_fails(client: AsyncClient):
    ...
    res = await client.patch(f"/api/v1/notifications/{uuid4()}", headers=headers)
    assert res.status_code == 404
...
async def test_delete_other_users_notification_fails(client: AsyncClient, db_session: AsyncSession):
    ...
    res = await client.delete(f"/api/v1/notifications/{notif_id}", headers=headers_b)
    assert res.status_code == 404
...
async def test_delete_nonexistent_notification_fails(client: AsyncClient):
    ...
    res = await client.delete(f"/api/v1/notifications/{uuid4()}", headers=headers)
    assert res.status_code == 404
...
async def test_delete_notification_unauthorized(client: AsyncClient):
    ...
    res = await client.delete(f"/api/v1/notifications/{uuid4()}")
    assert res.status_code == 401
