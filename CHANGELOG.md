...
- **`cleanup_expired_tokens` cron job** — scans Redis key prefixes (`blacklist:*`, `email_verify:*`, `password_reset:*`) and deletes orphaned keys (TTL=-1); runs nightly at midnight via ARQ cron
